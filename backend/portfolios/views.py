    ...
    try:
        ...
        for row_idx, row in enumerate(rows, start=2):
            try:
                ...
                # Extraire les champs
                date_transaction = row_data.get('Date')
                type_operation = row_data.get('Type', '').upper()
                isin = row_data.get('Isin') or row_data.get('ISIN') or row_data.get('isin')
                quantite = row_data.get('quantité') or row_data.get('Quantité') or row_data.get('quantite')
                prix_unitaire = row_data.get('prix unitaire') or row_data.get('Prix unitaire') or row_data.get('Prix Unitaire')
                ...
                # Validation
                if not all([date_transaction, type_operation, isin, quantite, prix_unitaire]):
                    erreurs.append({
                        'ligne': row_idx,
                ...
                try:
                    ...
                    # Créer une signalétique minimale si elle n'existe pas
